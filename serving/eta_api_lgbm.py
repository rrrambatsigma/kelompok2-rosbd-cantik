...
ES_INDEX = "flight_predictions_lgbm"

es = None

app = FastAPI(title="ETA Prediction API (LightGBM)", version="1.0.0")
...
def startup():
    ...
    try:
        ...
        print(f"[LGBM API] ES connection failed: {e}")
        es = None
...
def ensure_index():
    if not es.indices.exists(index=ES_INDEX):
        ...
        print(f"[LGBM API] Index '{ES_INDEX}' created.")
...
def list_predictions(
    ...
    method: Optional[str] = Query(None, description="Filter by prediction method (lgbm_classifier, callsign, heading_scoring)"),
...
def predict_endpoint(req: PredictRequest):
    try:
        from eta_pipeline_lgbm import predict
    ...
    if es is not None:
        try:
            es.index(index=ES_INDEX, id=req.icao24, body=result)
        except Exception as e:
            print(f"[LGBM API] Save error: {e}")
