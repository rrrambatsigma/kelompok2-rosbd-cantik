...
for i, route_name in enumerate(unique_routes):
    ...
    n_samples = min(len(route_data), MAX_SAMPLES_PER_ROUTE)
    sampled_route = route_data.sample(n=n_samples, random_state=RANDOM_STATE)
...
df["route"] = route_encoder.fit_transform(df["route"])

encoder_path = os.path.join(MODEL_DIR, "route_encoder_lgbm.pkl")
joblib.dump(route_encoder, encoder_path)
print("Route encoder saved:", encoder_path)
...
model_path = os.path.join(MODEL_DIR, "eta_lgbm.pkl")
