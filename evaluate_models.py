...
def evaluate():
    print("=" * 70)
    print("  MODEL EVALUATION (via Landing Events)")
    print("=" * 70)

    # Step 1: Find flights that recently landed
    ...
    # Step 2: Process each landing flight
    landings = []
    for icao, last_ts in landing_icaos:
        ...
        # Find transition point: last air point before on_ground
        ground_idx = None
        ...
        if ground_idx is None or ground_idx == 0:
            # Maybe the flight is still on ground, skip
            continue
        ...
        ground_pt = traj.iloc[ground_idx]
        ...
        # Actual ETA: from prediction point to landing
    ...
    dest_results = {
        "ml_classifier": {"correct": 0, "total": 0, "time": 0},
    ...
    for ld in landings:
        ...
        dest_results["ml_classifier"]["time"] += time.time() - t0
        dest_results["ml_classifier"]["total"] += 1
        if ml_result and ml_result["destination"] == gt:
            dest_results["ml_classifier"]["correct"] += 1
    ...
    n_dest = dest_results["ml_classifier"]["total"]
    print(f"\n  Samples: {n_dest}")
    print()

    for method, label in [("ml_classifier", "ml_classifier"),
    ...
    method_usage = {"ml_classifier": 0, "callsign": 0, "heading_scoring": 0, "failed": 0}
    for ld in landings:
        lat, lon = ld["pred_lat"], ld["pred_lon"]
        alt, hdg = ld["pred_alt"], ld["pred_hdg"]
        cs = ld["pred_callsign"]
        ...
        ml_r = predict_by_ml(lat, lon, alt, hdg)
        if ml_r:
            method_usage["ml_classifier"] += 1
            continue
    ...
    for m in ["ml_classifier", "callsign", "heading_scoring", "failed"]:
    ...
    eta_results = {"distance_speed": [], "xgboost": [], "route_avg": []}
    eta_samples = 0
    ...
    for ld in landings:
        ...
        actual_eta = ld["actual_eta_seconds"]

        if speed < 50 or alt < 100:
            continue

        # Ground truth ETA from the actual landing
        true_eta = actual_eta
    ...
    for method in ["distance_speed", "xgboost", "route_avg"]:
    ...
    ml_acc = dest_results["ml_classifier"]["correct"] / max(dest_results["ml_classifier"]["total"], 1) * 100
  ...
  Destination Accuracy:
    ml_classifier       : {ml_acc:.1f}%
    heading_scoring     : {hdg_acc:.1f}% (top-1), {top5_recall:.1f}% (top-5)

  Coverage (method used):
    ml_classifier       : {method_usage['ml_classifier']/total*100:.1f}%
